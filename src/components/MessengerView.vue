...
import MessengerGroupSheet from './MessengerGroupSheet.vue'
...
<template>
  <section class="space-y-3 pb-4">
    ...
    <MessengerGroupSheet
      v-if="groupSheetOpen"
      v-model:group-title="groupTitle"
      v-model:search-query="groupSearchQuery"
      :title="groupActionTitle"
      :subtitle="groupMode === 'add' ? activeConversation?.displayTitle : 'Название и участники'"
      :mode="groupMode"
      :selected-users="selectedGroupUsers"
      :search-users="groupSearchUsers"
      :selected-member-ids="selectedGroupMemberIds"
      :can-submit="canSubmitGroup"
      :busy="groupBusy"
